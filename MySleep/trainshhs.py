...
def train(
    ...
    # valid_sids = train_sids[:n_valids]
    ...
    for epoch in range(model.get_current_epoch(), config["n_epochs"]):
        ...
        if model.global_epoch > 81:
            ...
            # 模拟退火降温
            if model.T > model.Tf:
                model.T = 100 - math.pow(((model.global_epoch)), 2)
        ...
        # Check best model
        # if best_acc < test_outs["test/accuracy"] and \
        #    best_mf1 <= test_outs["test/f1_score"]:
        #     best_acc = test_outs["test/accuracy"]
        #     best_mf1 = test_outs["test/f1_score"]
        ...
        if (epoch == 99):
            ...
            writer = model.train_writer
            writer.add_scalar(tag="e_losses/test", scalar_value=best_outs["test/loss"],
                              global_step=train_outs["global_step"])
            writer.add_scalar(tag="e_losses/epoch", scalar_value=epoch + 1, global_step=train_outs["global_step"])
            writer.add_scalar(tag="e_accuracy/test", scalar_value=best_outs["test/accuracy"],
                              global_step=train_outs["global_step"])
            writer.add_scalar(tag="e_accuracy/epoch", scalar_value=epoch + 1, global_step=train_outs["global_step"])
            writer.add_scalar(tag="e_f1_score/test", scalar_value=best_outs["test/f1_score"],
                              global_step=train_outs["global_step"])
            writer.add_scalar(tag="e_f1_score/epoch", scalar_value=epoch + 1, global_step=train_outs["global_step"])
