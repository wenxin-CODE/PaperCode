...
def load_datan(prefix, normalize=True, load_walks=False):

    G_data = json.load(open(prefix + "ppi-G.json"))
    ...
    if os.path.exists(prefix + "ppi-feats.npy"):
        features1 = np.load(prefix + "ppi-feats.npy")
    else:
        print("No features present.. Only identity features will be used.")
        features1 = None
    id_map = json.load(open(prefix + "ppi-id_map.json"))
    id_map = {conversion(k): int(v) for k, v in id_map.items()}

    features1 = sp.csr_matrix(features1,dtype=np.float32)
    all_edges=G.edges
    ...
    adj = adj + adj.T.multiply(adj.T > adj) - adj.multiply(adj.T > adj)  # 顺序
    idx_train = range(10000)  # 训练集
    idx_val = range(10000, 40000)  # 验证集
    idx_test = range(40000, 50000)  # 测试集

    return features1, edges,adj,idx_train,idx_val,idx_test

def load_datam(path="../GIC/data/facebook/", dataset="facebook"):
    ...
    train_edges2 = edges.copy()  # 复制train_edges
    train_edges2[:, [0, 1]] = train_edges2[:, [1, 0]]  # 交换两个节点的位置
    train_edges = np.concatenate((edges, train_edges2), axis=0)  # 连接train_edges和train_edges2
    edges = train_edges
    adj = sp.coo_matrix((np.ones(edges.shape[0]), (edges[:, 0], edges[:, 1])),
                              shape=(features1.shape[0], features1.shape[0]),
                              dtype=np.float32)  # 邻接矩阵
    # build symmetric adjacency matrix
    adj = adj + adj.T.multiply(adj.T > adj) - adj.multiply(adj.T > adj)  # 顺序
    idx_train = range(112)  # 训练集
    idx_val = range(112, 162)  # 验证集
    idx_test = range(162, 212)  # 测试集
    return edges,features1,adj,idx_train,idx_val,idx_test
...
def load_datam1(path="../GIC/data/facebook/", dataset="facebook"):
    ...
    adj = adj + adj.T.multiply(adj.T > adj) - adj.multiply(adj.T > adj)  # 顺序
    idx_train = range(200)  # 训练集
    idx_val = range(200, 300)  # 验证集
    idx_test = range(300, 700)  # 测试集
    return edges, features1, adj, idx_train, idx_val, idx_test
...
def load_data(dataset_str): # {'pubmed', 'citeseer', 'cora'}
    ...
    return edges,features,adj,idx_train,idx_val,idx_test
def load_data_1():
    object_to_idx = {}
    idx_counter = 0
    edges = []
    ...
    #with open(os.path.join(data_path, "features.csv".format(dataset_str)), 'r') as f:
        #data = f.readlines()
    data = pandas.read_csv("./data/twitch/features.csv")
    data=np.array(data.loc[:, :])
    row =data[:,0]
    col = data[:,1]
    values = data[:,2]
    node_count = int(max(row))+1
    feature_count = int(max(col))+1
    ...
    # build symmetric adjacency matrix
    adj = adj + adj.T.multiply(adj.T > adj) - adj.multiply(adj.T > adj)  # 顺序
    idx_train = range(15000)  # 训练集
    idx_val = range(15000, 25000)  # 验证集
    idx_test = range(25000, 37000)  # 测试集
    return edges,features1,adj,idx_train,idx_val,idx_test
